...
export const insertSettlement = async(data, matchMult)=> {
    try{
        ...
        const winMult = matchMult >= Number(multiplier) ? Number(multiplier).toFixed(2) : 0.00 
        await write(`INSERT INTO settlement (bet_id, lobby_id, user_id, operator_id, bet_amount, max_mult, match_max_mult) VALUES(?, ?, ?, ?, ?, ?, ?)`, [data, matchId, decodeUserId, operatorId, parseFloat(betAmount), winMult, matchMult]);
